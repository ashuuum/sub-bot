...
async def add_subscription_db(user_id: int, name: str, cost: int, end_date: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def get_subscriptions_db(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT name, cost, end_date FROM subscriptions WHERE user_id = ?",
            (user_id,)
        )
...
async def update_subscription_db(user_id: int, old_name: str, new_name: str, cost: float, end_date: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            'UPDATE subscriptions SET name = ?, cost = ?, end_date = ? WHERE user_id = ? AND name = ?',
...
# --- Удаление подписки ---
async def del_subscription_db(user_id: int, name: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def get_expiring_subscriptions(date: str):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT user_id, name FROM subscriptions WHERE end_date = ?",
            (date,)
        )
        rows = await cursor.fetchall()
        return rows